...
import AdminStack from "./src/admin/navigation/AdminStack";

import AdminVerificationScreen from "./src/admin/screens/AdminVerificationScreen";
import "./global.css";
...
const RootNavigator = () => {
  ...
  const { isAuthenticated, role, loading } = useAuth(); // 4. Muestra un "cargando" mientras Firebase Auth se inicializa
  if (loading) {
    return (
  ...
  switch (role) {
    // 💡 NUEVA RUTA: ROL DE ADMINISTRADOR
    case "admin":
      return <AdminStack />; // Navega al panel de verificación

    case "patient":
      return <PatientAppStack />;
    case "nurse":
      return <NurseAppStack />;
    default: // Caso de error (Usuario logueado pero sin rol en Firestore)
      return (
        <View className="flex-1 justify-center items-center bg-error-rojo/10 p-10">
          ...
          <Text className="text-sm text-gray-600 mt-2">
            (Asegúrate de que el documento en la colección "users" tenga un
            campo "role".)
          </Text>
        </View>
      );
