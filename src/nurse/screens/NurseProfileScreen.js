...
const NurseProfileScreen = ({ navigation }) => {
 ...
 const updateFirestore = async (fieldName, newValue, updateBothCollections = false) => {
  ...
  try {
   ...
   Alert.alert("Éxito", `${fieldName} actualizado correctamente.`);
...
const handleEdit = (field, label, isContact = false) => {
    // 💡 CAMBIO AQUÍ: Ahora mostramos el mensaje de error para especialidad
  ...
  <SafeAreaView className="flex-1 bg-fondo-claro">
   ...
   <ScrollView className="flex-1 p-4">
    ...
    <View className="items-center py-6 bg-white rounded-xl shadow-md mb-6 border border-gris-acento">
     ...
     <TouchableOpacity onPress={pickImage} className="mt-3">
      <Text className="text-az-primario font-semibold text-base">Cambiar Foto</Text>
     </TouchableOpacity>
   ...
   <View className="flex-row justify-around items-center bg-white border-t border-gris-acento pt-2 pb-4 shadow-xl">
    ...
    <EditFieldModal
        ...
        onSave={(newValue) => {
            // Llama a la función de guardado con la información del campo
